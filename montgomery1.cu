...
xDBL(env_t env,
  ...
  const typename env_t::cgbn2_t a = curve.a;
  const typename env_t::cgbn2_t b = curve.b;
  ...
  typename env_t::cgbn2_t t0, t1, t2;
...
j_inv(env_t env,
  ...
  const typename env_t::cgbn2_t a = E.a;
  typename env_t::cgbn2_t t0, t1;
...
get_yP_yQ_A_B(env_t env,
    ...
    mont_curve_int_t<env_t>& curve) {
  typename env_t::cgbn2_t a = curve.a;
  typename env_t::cgbn2_t b = curve.b;
  mont_pt_t<env_t> P = curve.P;
  mont_pt_t<env_t> Q = curve.Q;

  const typename env_t::cgbn2_t xP = pk.xP;
  const typename env_t::cgbn2_t xQ = pk.xQ;
  const typename env_t::cgbn2_t xR = pk.xR;

  mont_pt_t<env_t> T;
  mont_pt_init_set_ui32<env_t>(env, p, 0U, 0U, 0U, 0U, T);
  typename env_t::cgbn2_t t1 = T.x;
  typename env_t::cgbn2_t t2 = T.y;
